...
const PersonnelPage = () => {
  ...
  const [allList, setAllList] = useState<any>([...data1List, ...data2List, ...data3List, ...data4List]);
  ...
  const handChangeSwitch = (checked: boolean, t:any) => {
   setDataList(tableList.map(item =>{
        ...
        <div className={styles.pageContLeft}>
          <div className={styles.list1}>
            <div  className={styles.listCont}>
              {data1List.map(t => {
                return <div 
                  className={`${styles.listItem} ${t.id === obj.id ? styles.listItemActive : ''} ${t.id === nextId ? styles.listItemNext : ''}`}
                  onClick={()=> handItem(t)}
                  key={t.id}>
                  <div className={styles.listItemImg}></div>
                  <div className={styles.listItemTitle} title={t.name}>{t.name}</div>
                </div>
              })}
            </div>
          </div>
        ...
        <div className={styles.pageContRight}>
          <div className={styles.rightTop}>
            ...
            <div className={styles.rightTopGroup}>
              <img 
                ...
                alt="" />
               <span className={styles.rightTopGroupTitle} title={obj.name}>{obj.name}</span>
              <img
          ...
          <div className={styles.table}>
              <div className={styles.tableTop}>
                <div className={`userform ${styles.tableTopLfet}`}>
                  <Input.Search 
